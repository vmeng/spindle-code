...
FULLTEXT_RSS_FILENAME = os.path.join(settings.SPINDLE_PUBLIC_DIRECTORY,
                                     settings.SPINDLE_FULLTEXT_RSS_FILENAME)

# URL for published RSS feed
EXPORTS_RSS_URL = urlparse.urljoin(PUBLIC_URL,
                                   settings.SPINDLE_EXPORTS_RSS_FILENAME)
FULLTEXT_RSS_URL = urlparse.urljoin(PUBLIC_URL,
                                    settings.SPINDLE_FULLTEXT_RSS_FILENAME)
...

#
# An RSS feed containing full plaintext transcripts
#
class FulltextFeed(SpindleFeed):
    """RSS feed containing the full plaintext of each exported track."""
    def add_item_elements(self, handler, entry):
        super(FulltextFeed, self).add_item_elements(handler, entry)
        ...
        self.add_item_original_guid_links(handler, export)
        self.add_item_category_tags(handler, export)
        self.add_plaintext_content(handler, export)

    def add_plaintext_content(self, handler, export):
        try:
            with open(export.filepath, 'r') as textfile:
                content =  ' '.join(list(textfile))
                logger.info(content[0:100])
                handler.addQuickElement('atom:content', content,
                                        attrs = { 'type': 'text' })
        except IOError:
            pass

@single_instance_task(name='spindle.publish.fulltext_feed',
                      cache_id='export_fulltext_feed_task_id',
                      logger=logger)
def publish_fulltext_feed(debug=False):
    feed = FulltextFeed(title = "Spindle fulltext transcripts",
                          link = "/",
                          description = "Plain text transcripts generated by Spindle")
    items = Item.objects.filter(track_count__gt=0).order_by('-updated')
    if debug: items = items[0:10]

    logger.info(u"URL = {}\nPath = {}".format(FULLTEXT_RSS_URL, FULLTEXT_RSS_FILENAME))
    total = len(items)
    for index, item in enumerate(items):
        publish_fulltext_feed.update_progress(float(index) / total, item.name)

        for export in item_exports(item):
            if export.mime_type == 'text/plain' and export.file_exists:
                # Force computation of keywords inside this loop if
                # necessary.  This allows for more accurate progress
                # updates.
                keywords = export.track.keywords
    ...
    with open(FULLTEXT_RSS_FILENAME, 'wb') as outfile:
        feed.write(outfile, 'utf-8')
